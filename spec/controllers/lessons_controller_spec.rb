...
RSpec.describe LessonsController, type: :controller do
  ...
  describe "GET #index" do
    ...
    it 'has lessons in the lessons instance variable' do
      3.times { |index| Lesson.create(subject: "#{index}logy", teacher: "Mrs_#{index}", bully: "Mark", bullied: "Markie") }
      get :index, :kindergarten_id => lesson.kindergarten_id
  ...
  describe "POST #create" do
    ...
    before(:all) do
      @lesson_params = { lesson: { subject: 'Test', teacher: 'Mr_Test', bully: 'X', bullied: 'Y'}}
    end
    ...
    it "sets a flash message on error" do
      post :create, { :kindergarten_id => lesson.kindergarten_id, id: lesson.id, lesson: { subject: 'Test', teacher: 'Mr_Test', bully: 'X', bullied: 'Y'}}
  ...
  describe 'PUT #update' do
    ...
    it "sets the lesson instance variable" do
      put :update, { :kindergarten_id => lesson.kindergarten_id, id: lesson.id, lesson: { subject: 'New lesson', teacher: 'Mr_Test', bully: 'X', bullied: 'Y' }}
      expect(assigns(:lesson).id).to eq(lesson.id)
    end

    it "updates the lesson" do
      put :update, { :kindergarten_id => lesson.kindergarten_id, id: lesson.id, lesson: { subject: 'New lesson', teacher: 'Mr_Test', bully: 'X', bullied: 'Y'}}
      expect(lesson.reload.name).to eq('New lesson')
    end

    it "sets a flash message on success" do
      put :update, { :kindergarten_id => lesson.kindergarten_id, id: lesson.id, lesson: { subject: 'New lesson', teacher: 'Mr_Test', bully: 'X', bullied: 'Y'}}
      expect(flash[:success]).to eq('lesson Updated Succesfully!')
    end

    it "redirect to show on success" do
      put :update, { :kindergarten_id => lesson.kindergarten_id, id: lesson.id, lesson: { subject: 'New lesson', teacher: 'Mr_Test', bully: 'X', bullied: 'Y'}}
      expect(response).to redirect_to(kindergarten_lesson_path(lesson.kindergarten_id, lesson.id))
    end

    describe 'update failures' do

      it "renders edit on fail" do
        put :update, { :kindergarten_id => lesson.kindergarten_id, id: lesson.id, lesson: { make: nil, model: 'STI', year: '2004', price: '1000', engine: 2000, c_type: 'sedan' } }
        expect(response).to render_template(:edit)
      end
